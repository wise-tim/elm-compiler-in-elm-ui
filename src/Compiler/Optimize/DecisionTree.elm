...
type DecisionTree
    = Match Int
    | Decision {- path -} Path {- edges -} (TList ( Test, DecisionTree )) {- default -} (Maybe DecisionTree)
...
type Branch
    = Branch {- goal -} Int {- patterns -} (TList ( Path, Can.Pattern ))


getPatterns (Branch _ patterns) =
    patterns
...
isComplete tests =
    case List.head tests |> Maybe.withDefault IsTuple of
...
checkForMatch branches =
    case branches of
        (Branch goal patterns) :: _ ->
            if MList.all (not << needsTests << Tuple.second) patterns then
                Just goal
...
testAtPath selectedPath (Branch _ pathPatterns) =
    MMaybe.bind (MList.lookup selectedPath pathPatterns) <|
        \(A.At _ pattern) ->
            case pattern of
...
-- TODO "aliases should never reach 'testAtPath' function"
-- BUILD EDGES
...
-- TODO "aliases should never reach 'isIrrelevantTo' function"
-- PICK A PATH
...
pickPath branches =
    ...
    case bests (addWeights (smallDefaults branches) allPaths) of
        ...
        tiedPaths ->
            List.head (bests (addWeights (smallBranchingFactor branches) tiedPaths))
...
bests allPaths =
    case allPaths of
        ...
        -- TODO "Cannot choose the best of zero paths. This should never happen."
