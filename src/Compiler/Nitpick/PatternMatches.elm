    ...
    case toNonRedundantRows region patterns of
        ...
        Right matrix ->
            case isExhaustive matrix 1 of
                ...
                badPatterns ->
                    Incomplete region context (List.filterMap List.head badPatterns) :: errors
                        ...
                        ((Can.Union _ altList numAlts _) as alts) =
                            Tuple.second
                                (Dict.toList ctors
                                    ...
                                    |> Maybe.withDefault ( "Compiler Bug! empty ctors dict", Can.Union [] [] -1 Can.Normal )
                                )
                    in
...
-- TODO "Compiler error! Empty matrices should not get specialized."
    ...
    if numSeen == 0 then
        ...
        let
            (Can.Union _ alts numAlts _) =
                Tuple.second (Dict.toList ctors |> List.Extra.minimumBy Tuple.first |> Maybe.withDefault ( "", Can.Union [] [] -1 Can.Normal ))
        in
        if numSeen == numAlts then
            Yes alts
