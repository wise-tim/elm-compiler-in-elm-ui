...
toDiff3 localizer ctx tipe1 tipe2 =
    case ( tipe1, tipe2 ) of
        ...
        ( Type home1 name1 args1, Type home2 name2 args2 ) ->
            if home1 == home2 && name1 == name2 then
                fmap (RT.apply ctx (L.toDoc localizer home1 name1)) <|
                    MList.sequenceA pure liftA2 (MList.zipWith (toDiff localizer RT.App) args1 args2)
        ...
        ( Alias home1 name1 args1 _, Alias home2 name2 args2 _ ) ->
            if home1 == home2 && name1 == name2 then
                fmap (RT.apply ctx (L.toDoc localizer home1 name1)) <|
                    MList.sequenceA pure liftA2 (MList.zipWith (toDiff localizer RT.App) (MList.map Tuple.second args1) (MList.map Tuple.second args2))
    ...
    case iteratedDealias tipe of
            ...
            case super of
                ...
                Comparable ->
                    isInt h n
                        ...
                        || isList h n
                        && (List.head args
                                |> Maybe.map (\head -> isSuper super head)
                ...
                CompAppend ->
                    isString h n
                        || isList h n
                        && (List.head args
                                |> Maybe.map (\head -> isSuper Comparable head)
                                |> Maybe.withDefault False
                           )
        ...
        Tuple a b maybeC ->
            case super of
                ...
                Comparable ->
                    isSuper super a && isSuper super b && MMaybe.maybe True (isSuper super) maybeC
...
diffRecord localizer fields1 ext1 fields2 ext2 =
    let
        ...
        left =
            Map.mapWithKey toUnknownDocs (Map.difference fields1 fields2)

        both =
            Map.intersectionWithKey toOverlapDocs fields1 fields2

        right =
            Map.mapWithKey toUnknownDocs (Map.difference fields2 fields1)
            ...
            case ( hasFixedFields ext1, hasFixedFields ext2 ) of
                ( True, True ) ->
                    case Map.lookupMin left of
                        Just ( f, _ ) ->
                            Different <| Bag.one <| FieldTypo f (Map.keys fields2)

                        Nothing ->
                            if Map.null right then
                                Similar

                            else
                                Different <| Bag.one <| FieldsMissing (Map.keys right)
                ...
                ( True, False ) ->
                    case Map.lookupMin right of
                        Just ( f, _ ) ->
                            Different <| Bag.one <| FieldTypo f (Map.keys fields1)
